...
static int sendConnectionReply(participantsDb_t db,
	...
	reply.clNr = 
	    htonl(udpc_addParticipant(db,
				      ...
				      capabilities,
				      rcvbuf,
				      config->flags & FLAG_POINTOPOINT, sock)); /* wangq add 'sock' 20140825*/
...
#define FILE_IPART 				"/tmp/num_of_ipart.txt"
#define FILE_NUM_OF_DB			"/tmp/num_of_db.txt"	
#define FILE_START_SIGNAL		"/tmp/start_signal.txt"
#define FILE_IP_STATIC			"/tmp/ip_static.txt"
#define FILE_PARTION_NUM 		"/tmp/partion_num.txt" //mod 20140414
#define FILE_PARTION_NUM1 		"/tmp/partion_num1.txt" //mod 20140414
#define FILE_PARTS				"/tmp/file_parts.txt"
#define FILE_OCS_END			"/tmp/file_ocs_end.txt"

static int
rcd_check_file_exist(char *fm)
{	
	int tmp, tmp_t;
	tmp = access(fm, 0);
	if(tmp != 0)
		tmp_t = 1;//file not exist
	else	
		tmp_t = 0;//file exist
	return tmp_t;
}
...
static int
rcd_read_file(char *fm)
{ 
	int num = -1; 
	FILE* fp;
	char n[4];
	if((fp = fopen(fm, "r")) == NULL)
	{
		return -1;
	}
	fgets(n, 5, fp);
...
static int
rcd_create_file(int num, char *fm)
{
        FILE* fp;

        if((fp = fopen(fm, "w+")) == NULL){
        	return -1;
    	}
...
rcd_read_parts(void)
{
	...
	if((fp=fopen(FILE_PARTS, "r"))==NULL)
    	{
       	return -1;
    	}
    	fgets(arDest, max_size, fp);

    	for(szGet = strtok(arDest, arZe); szGet != NULL; szGet = strtok(NULL, arZe))
    	{
        	strcpy(arGet, szGet);
        	if((ret = strcmp(arGet, "\n")) == 0){
                	return 0;
...
#if 0
static int
rcd_read_partion_num(void)
{
	int num = -1; 
	FILE* fp;
	char n[4];
	
	if((fp = fopen(FILE_PARTION_NUM, "r")) == NULL)
	{
		return -1;
	}
			   ...
			   time_t *firstConnected)
{
	...
	int ret;
	int num;
	ret = rcd_check_file_exist(FILE_IPART);
	if(ret != 0)
		return 0;
	ipart = rcd_read_file(FILE_IPART);
       if(ipart == 1){
	   	ret = rcd_check_file_exist(FILE_START_SIGNAL);
		if(ret != 0){
			return 0;
		}else{
			rcd_create_file( nrParticipants(db), FILE_NUM_OF_DB);
			return 1;
		}			
       }else{
		num = rcd_read_file(FILE_NUM_OF_DB);
		//ret = rcd_check_file_exist(FILE_START_SIGNAL);
		//if(ret != 0)
			//return 0;
       }
...
/************************/
	...
	ipart1 = rcd_read_file(FILE_IPART);
	npart = rcd_read_parts();
	if(ipart1 == npart){
		rcd_create_file1(FILE_OCS_END);
	}
/************************/
    return 0;
}
